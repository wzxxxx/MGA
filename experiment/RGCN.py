...
args['select_task_list'] = ['CYP1A2']  # change
...
for task in args['select_task_list']:
    if task in ['Carcinogenicity', 'Ames Mutagenicity', 'Eye irritation',
                'Eye corrosion', 'CYP1A2', 'CYP2C19', 'CYP2C9', 'CYP2D6', 'CYP3A4',
                'NR-AR', 'NR-AR-LBD', 'NR-AhR', 'NR-Aromatase', 'NR-ER', 'NR-ER-LBD',
                'NR-PPAR-gamma', 'SR-ARE', 'SR-ATAD5', 'SR-HSE', 'SR-MMP', 'SR-p53']:
        args['classification_num'] = args['classification_num'] + 1
    if task in ['Acuteoraltoxicity(LD50)', 'LC50DM', 'BCF', 'LC50', 'IGC50']:
...
result_pd.to_csv('../ressult/' + args['task_name']+'_result.csv', index=None)
